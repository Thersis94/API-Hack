...
$("form").on("submit", function(event) {
  ...
  if (userHighLight.length >= 1 && userInput.length > 0) {
    userHighLight = userHighLight.join(",");
    fetchAPI(userHighLight + "," + userInput);
  }
  if (userHighLight.length >= 1 && userInput.length == 0) {
    fetchAPI(userHighLight)
  }
  console.log(userInput.length)
  if (userHighLight.length === 0 && userInput.length > 0) {
    fetchAPI(userInput)
  }
  if (userHighLight.length === 0 && userInput.length === 0) {
    console.log('running')
...
function fetchAPI(userInput) {
  let API_ENDPOINT = 'https://www.thecocktaildb.com/api/json/v2/8673533/'
  let request = null
  if(userInput === '') {
    userInput = 'search.php?f=a'
    request = API_ENDPOINT + userInput
    console.log(request)
  }
  else {
    userInput = 'filter.php?i=' + userInput
    request = API_ENDPOINT + userInput
    console.log(request)
  }
  return fetch(
